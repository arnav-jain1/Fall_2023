...
{
    intr_count++;
    set_cycles(get_cycles() + 3277); //Sets the cycle to current + hertz/10 (rounds to 3277)
    // YOUR CODE HERE
}
...
void enable_interrupt()
{
    ...
    write_csr(mstatus, read_csr(mstatus) | (1 << MSTATUS_MIE_BIT)); //Sets the interrupt bit to 1 in mstatus register
}
...
void disable_interrupt()
{
    ...
    /* Look at the enable_timer_interrupt() function for hints about
       how to write this function */
    write_csr(mstatus, read_csr(mstatus) & ~(1 << MSTATUS_MIE_BIT)); //Sets the interrupt bit to 0 in mstatus register; Does this by shifting 1 and then notting the whole thing
